/* Miscellaneous javascript routines by DA/AD */
...
function number2locale(n, skipZeroCents, roundCents) {
    ...
    if ((typeof roundCents) != "undefined" && roundCents) {
...
function showCost(cost, skipZeroCents, skipNonZeroCents) {
    ...
    var c = "";
    var d = 1;
    if (cost > 1000000) { c = "M"; d = 1000000; }
    if (cost > 1000000000) { c = "B"; d = 1000000000; }
...
function htmlGraphBarWithValue(dValue, MaxValue, sValueText, ImagesPath, BWidth, BHeight, sColor, sGradient, sClickHandler) {
    var tMargin = 0;
    var sFill = "";
    var FillWidth = -1;
    ...
    if (FillWidth > 0) sBG += (sBG = "" ? " background:" : "") + " " + sColor;
    var sTextCol = "#000000";
    if (FillWidth > 0 && dValue >= 0.65) sTextCol = contrastColor(sColor); // use contrast color if bar width > 65%
    //if ((typeof sGradient) != 'undefined') sBG = " background: " + sGradient + " no-repeat; background-size:" + FillWidth + "px " + BHeight + "px; ";
    var sBar = "<div class='bar_value' style='display:block; font-size:10px; line-height:" + sLH + "%; width:100%; height:100%;" + sBG + "' title='" + sValueText + "'><font style='color:" + sTextCol + "; vertical-align: middle;'>" + sValueText + "</font></div>";
...
function _ajaxSend(params, url) {
    ...
    var _ajax_request = $.ajax({
        ...
        error: function (jqXHR, textStatus) { if (typeof _ajaxOnEvent != "undefined")  _ajaxOnEvent([jqXHR, textStatus], _ajax_error_code); if (_ajax_show_error) DevExpress.ui.dialog.alert(_ajax_error_msg, "Error"); },
...
/*    }
        ...
        jDialog("Error on processing response!", true, ";", "undefined");
    }*/
    //-A1228 _ajax_ok_code = "";
...
function dxDialog(msg, yes_code, no_code, title, btn_ok_text, btn_no_text) {
    var buttons = [{
        ...
        text: (btn_ok_text) ? btn_ok_text : resString("btnOK"),
        onClick: function () {
            return true;
        }
    }];
