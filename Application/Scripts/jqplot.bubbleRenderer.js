/* AD: fixes for avoid crashing on mouse over and click */
/* DA: shapes support */
/**
* jqPlot
...
(function ($) {
    ...
    */
        ...
        for (var i = 0; i < this.radii.length; i++) {
            ...
            if (d[3]) {
                if (typeof (d[3]) == 'object') {
                    t = d[3]['label'];
                    sh = d[3]['shape']; /*DA*/
                    brd = d[3]['guid'].indexOf("-") == 0 ? "- -" : ""; /*DA*/
                    if (sh >= 0) { this.shadow = false; shadow = false; opts.shadow = false; }; /*DA*/
                }
            ...
            gd.ctx = ctx; /*DA*/
            this.bubbleCanvases[idx].draw(gd[2], color, this.bubbleGradients, this.shadowAngle / 180 * Math.PI, sh, brd); /*DA*/
    ...
    $.jqplot.BubbleCanvas.prototype.draw = function (r, color, gradients, angle, shape, border) { /*DA*/
        var ctx = this._ctx;
        // r = Math.floor(r*1.04);
        // var x = Math.round(ctx.canvas.width/2);
        // var y = Math.round(ctx.canvas.height/2);
        var w = ctx.canvas.width; /*DA*/
        var h = ctx.canvas.height; /*DA*/
        var x = w / 2; /*DA*/
        var y = h / 2; /*DA*/
        ctx.save();
        if (gradients && !$.jqplot.use_excanvas) {
            ...
            ctx.fillRect(0, 0, w, h); /*DA*/
        }
        else {
            ctx.fillStyle = color;
            ctx.strokeStyle = color;
            ctx.lineWidth = 1;
            /*DA*/if (typeof (shape) != 'undefined' && shape != null && shape >= 0) {
                ctx.lineWidth = 2;
                if ((border) && border == "- -") ctx.setLineDash([5, 5]);
                switch (shape) {
                    case 0: // no category                        
                        //ctx.strokeStyle = "#cccccc";
                        ctx.beginPath();
                        var ang = 2 * Math.PI;
                        ctx.arc(x, y, r - 2, 0, ang, 0);
                        ctx.closePath();
                        if ((border) && border == "- -") {
                            setDashLine(ctx);
                        }
    ...
    function highlight(plot, sidx, pidx) {
        ...
        var border = "";
        if (s.data[pidx][3]) {
            if (typeof (s.data[pidx][3]) == 'object') {
                shape = s.data[pidx][3]['shape'];
                border = s.data[pidx][3]['guid'].indexOf('-') == 0 ? "- -" : "";
            }
        }

        if (typeof (shape) != 'undefined' && shape != null && shape >= 0) {
            var bctx = s.gridData[pidx].ctx;
            var w = bctx.canvas.width;
            var h = bctx.canvas.height;
            var x0 = x - w / 2;
            var y0 = y - h / 2;
            bctx.lineWidth = 2;
            ...
            switch (shape) {
                ...
                case 5: // label
                    ctx.beginPath();
                    ctx.moveTo(x0, y0);
                    ctx.lineTo(x0 + w * 0.6, y0);
                    ctx.lineTo(x0 + w - 1, y);
                    ctx.lineTo(x0 + w * 0.6, y0 + h);
                    ctx.lineTo(x0, y0 + h);
                    ctx.lineTo(x0, y0);
                    ctx.fill();
                    if ((border) && border == "- -") {
                ...
                case 8: // circle with plus
                    ...
                    var ang = 2 * Math.PI;
                    ctx.arc(x, y, r - 2, 0, ang, 0);
                    ctx.moveTo(x0 + w * 0.2, y);
                    ctx.lineTo(x0 + w * 0.8, y);
                    ctx.moveTo(x, y0 + h * 0.2);
                    ctx.lineTo(x, y0 + h * 0.8);
                    if ((border) && border == "- -") {
                        setDashLine(ctx);
                    }
                    ctx.stroke();
