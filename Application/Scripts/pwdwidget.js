/* Enhanced by AD // ExpertChoice Inc., 190615
*
* Password Widget 1.0
*
...
function _MakePWDWidget()
{
	...
	this.pwdclrdiv = pwdname + "_clrdiv";

	this.pwdclear_anch = pwdname + "_clr_anch";

	code += "<div class='pwdopsdiv' id='" + this.pwdclrdiv + "'><a href='' id='" + this.pwdclear_anch + "'>" + this.txtClear + "</a></div>";
	...
	this._clearPwd = clearPwd;

	this.clear_anch_obj = document.getElementById(this.pwdclear_anch);

	this.clear_anch_obj.pwdwidget = this;

	this.clear_anch_obj.onclick = function () { this.pwdwidget._clearPwd(); return false; }
	...
	this.pwdtxtfield_obj.onmouseup=function(){ setTimeout('w = document.getElementById("' + this.id + '"); if ((w)) w.pwdwidget._onKeyUpPwdFields();', 100); }
...
function passwordStrength()
{
	...
	if (password.length>4 && password.match(/\d+/)){ score++;}
	...
	{
		strengthdiv.style.width=Math.round(100/6*(score+1))+'%';
	}
...
function clearPwd() {
    this.pwdfieldobj.value = "";
    this.pwdtxtfield_obj.value = "";
