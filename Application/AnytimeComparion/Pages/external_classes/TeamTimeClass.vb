...
Public Class TeamTimeClass
    ...
    Public Shared Function LanguagesScanFolder(ByVal sPath As String) As List(Of clsLanguageResource)
        ...
        Dim AllFiles = New ObjectModel.ReadOnlyCollection(Of String)(files)
        Login.debugging = ""

        For Each sFileName As String In AllFiles

            If IO.Path.GetExtension(sFileName).ToLower() = Consts._FILE_RESOURCE_EXT And Not System.IO.Path.GetFileName(sFileName).StartsWith("~") Then
                Login.debugging += (sFileName & " <br>")
    ...
    Public Shared Sub set_SessVar(ByVal sName As String, ByVal value As String)
        ...
        End If
    End Sub
    Public Shared Function GetUserTemplateReplacements(fDirectReplace As Boolean, Optional tStepNode As clsNode = Nothing) As Dictionary(Of String, String)
    ...
    Public Shared Function GetAction(ByVal stepNumber As Integer) As clsAction
        Dim context As HttpContext = HttpContext.Current
        Dim App = Anytime.AppSession()
        Dim pipeCount = If(TeamTime Is Nothing, 0, TeamTime.Pipe.Count)

        If stepNumber < 1 Or stepNumber > pipeCount Or pipeCount < 1 Then
            Return Nothing
        Else
            Return CType(TeamTime.Pipe(stepNumber - 1), clsAction)
        End If
    End Function
    ...
    Public Shared Function getnodes(ByVal [step] As Integer) As List(Of Object)
        Dim context As HttpContext = HttpContext.Current
        Dim App = Anytime.AppSession()
        Dim Project = CType(TeamTime, clsTeamTimePipe)
        ...
        Dim FirstRating = CType(Nothing, clsNode)
        Dim SecondRating = CType(Nothing, clsNode)
        Dim ChildNode = CType(Nothing, clsNode)
                    ...
                    If ActionData2.Node IsNot Nothing AndAlso ActionData2.Judgment IsNot Nothing Then
                        ...
                        Select Case (CType(tAction.ActionData, clsNonPairwiseEvaluationActionData)).MeasurementType
                            ...
                            Case ECMeasureType.mtStep
                                ...
                                childinfo = CStr(InfodocService.Infodoc_Unpack(App.ProjectID, App.ActiveProject.ProjectManager.ActiveHierarchy, If(ChildNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), ChildNode.NodeID.ToString(), ChildNode.InfoDoc, True, False, -1))
                                parentinfo = CStr(InfodocService.Infodoc_Unpack(App.ProjectID, App.ActiveProject.ProjectManager.ActiveHierarchy, If(ParentNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), ParentNode.NodeID.ToString(), ParentNode.InfoDoc, True, False, -1))
                                wrtinfo = CStr(InfodocService.Infodoc_Unpack(App.ProjectID, App.ActiveProject.ProjectManager.ActiveHierarchy, Consts.reObjectType.AltWRTNode, ChildNode.NodeID.ToString(), App.ActiveProject.ProjectManager.InfoDocs.GetNodeWRTInfoDoc(ChildNode.NodeGuidID, ParentNode.NodeGuidID), False, True, ParentNode.NodeID))
                                list1.Add(childinfo)
                                list1.Add(parentinfo)
                                list1.Add(wrtinfo)
                                list1.Add("")
    ...
    Public Shared Function SetClusterPhraseForNode(ByVal NodeID As Guid, ByVal sClusterPhrase As String, ByVal IsMulti As Boolean, ByVal tAdditionlGuid As Guid) As Boolean
        'NodeID = New Guid("aad859c9-c87c-423a-b853-cf0f580f7d6b")
        Dim fResult = False
        Dim context As HttpContext = HttpContext.Current
        Dim App = Anytime.AppSession()
        Dim _with1 = App.ActiveProject.ProjectManager
        Dim attr As Attributes.clsAttribute = Nothing
