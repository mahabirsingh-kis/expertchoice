...
Public Class AnytimeClass
    ...
    Public Shared Sub CheckProjectIsAccessible(ByVal project As clsProject, ByVal email As String)
        ...
        If message.Length > 0 Then
            ...
            HttpContext.Current.Response.Redirect("~/AnytimeComparion/Default.aspx?accError=cannotAccessProject")
            'HttpContext.Current.Response.Redirect("~/AnytimeComparion/pages/Anytime/Anytime.aspx")
        End If
    End Sub
    ...
    Public Shared Function GetMessageIfProjectIsOfflineOrArchived(ByVal project As clsProject) As String
        ...
        If Not project.isPublic And HttpContext.Current.Session("NewUser") IsNot Nothing AndAlso HttpContext.Current.Session("NewUser").ToString() = "True" Then
            message = String.Format(TeamTimeClass.ResString("msgDisabledPasscode"), project.ProjectName)
        ElseIf Not project.isOnline Then
    ...
    Friend Shared Function GetComparionHashLink() As String
        ...
        Dim comparionUrl = If(url.Host.Equals("localhost"), $"{url.Scheme}://{url.Host}:20180/AnytimeComparion/Pages/Anytime/Anytime.aspx", $"{url.Scheme}://{url.Authority}/")
...
_Select1_CaseDefault:
        ...
        Key .showNormalization = Not app.ActiveProject.ProjectManager.Parameters.EvalHideLocalNormalizationOptions,
        Key .ShowBars = app.ActiveProject.ProjectManager.Parameters.ResultsLocalShowBars,
                ...
                If answer IsNot Nothing Then
                    ...
                    Select Case question.Type
                        ...
                        Case QuestionType.qtObjectivesSelect
                            ...
                            For Each node As clsNode In app.ActiveProject.HierarchyObjectives.Nodes
                                If node.DisabledForUser(tUser.UserID) Then
                                    AllAllowed = False
                                    Exit For
                                End If
                            Next
        ...
        If surveyPage IsNot Nothing AndAlso surveyPage.Survey IsNot Nothing Then
            ...
            If respondent IsNot Nothing Then
                ...
                If True Then
                    ...
                    tSurvey.SurveyDataProvider(respondent.Email) = Nothing
                End If
            End If
        End If
    ...
    Public Shared Sub ReadAnswer(ByVal answer As clsAnswer, ByVal question As clsQuestion, ByVal key As String)
        ...
        If question.Type <> QuestionType.qtComment Then
            ...
            If question.Type = QuestionType.qtObjectivesSelect Then
                Dim objectives = key.Split(CType(";", Char()))
                ...
                For Each node As clsNode In objectiveshierarchy
                    ...
                    If Not node.IsAlternative Then
                        ...
                        node.DisabledForUser(hierarchy.ProjectManager.User.UserID) = Nothing
                        If Not Convert.ToBoolean(objectives(idx + 1)) Then
                            If Not node.DisabledUserIDsList.Contains(hierarchy.ProjectManager.User.UserID) Then
                                node.DisabledUserIDsList.Add(hierarchy.ProjectManager.User.UserID)
                            End If
                        Else
                            node.DisabledUserIDsList.Remove(hierarchy.ProjectManager.User.UserID)
                        End If
                    End If
                Next
            End If
            ...
            If question.Type = QuestionType.qtAlternativesSelect Then
                ...
                Dim alternatives = key.Split(CType(";", Char()))
                Dim altshierarchy = app.ActiveProject.HierarchyAlternatives.Nodes

                If alternatives.Length >= altshierarchy.Count() Then
                    For Each node As clsNode In app.ActiveProject.HierarchyAlternatives.Nodes

                        If node.IsAlternative Then
                            Dim idx = altshierarchy.IndexOf(node)
                            node.DisabledForUser(hierarchy.ProjectManager.User.UserID) = Nothing
                            If Not Convert.ToBoolean(alternatives(idx + 1)) Then
                                If Not node.DisabledUserIDsList.Contains(hierarchy.ProjectManager.User.UserID) Then
                                    node.DisabledUserIDsList.Add(hierarchy.ProjectManager.User.UserID)
                                End If
                            Else
    ...
    Public Shared Function CreateGlobalResults(ByVal [step] As Integer, ByVal Optional normalization As Integer = 1, ByVal Optional customWrtNodeID As Integer = -1) As Object
        Try
            ...
            app.ActiveProject.ProjectManager.CheckCustomCombined()
            Dim dataobject = Anytime.GetOverallResultsData(normalization, customWrtNodeID, False)
            ...
            Key .showNormalization = Not app.ActiveProject.ProjectManager.Parameters.EvalHideGlobalNormalizationOptions,
            Key .ShowBars = app.ActiveProject.ProjectManager.Parameters.ResultsGlobalShowBars,
            Key .columnValueCombined = columnValueCombined,
    ...
    Public Shared Function set_collapse_cookies(node_type As String, output As AnytimeOutputModel) As Object
        Dim is_hidden = If((node_type + "-info-div").Contains("visible"), 1, 0)
        Dim bool_hidden = If((node_type + "-info-div").Contains("visible"), True, False)
        Dim is_multi = False
        If (output.page_type = "atNonPWAllChildren" Or output.page_type = "atNonPWAllCovObjs" Or output.page_type = "atAllPairwise" Or output.page_type = "atAllPairwiseOutcomes" Or output.page_type = "atShowLocalResults") Then
            is_multi = True
        End If
        Dim currentStepType = If(is_multi, "All_", "One_") + output.pairwise_type + output.non_pw_type
        If (node_type = "parent-node" And Not is_multi And output.page_type = "atPairwise") Then
            If (node_type = "left-node" Or node_type = "right-node" Or node_type = "parent-node") Then
                node_type = "parent-node"
            Else
                node_type = "pw-nodes"
            End If
        ElseIf (node_type = "wrt-left-node" Or node_type = "wrt-right-node") Then
            If (output.non_pw_type = "mtDirect" Or output.non_pw_type = "mtRatings" Or currentStepType = "One_mtStep" Or currentStepType = "One_mtRegularUtilityCurve") Then
                node_type = "pw-nodes"
            Else
                node_type = "pw-wrt-nodes"
            End If
        End If
        setCollapseCookies(currentStepType, node_type, is_hidden.ToString())
        If (node_type = "pw-nodes" Or node_type = "parent-node") Then
            If is_multi Then
                output.multi_collapse_default(0) = bool_hidden
                output.multi_collapse_default(1) = bool_hidden
                output.multi_collapse_default(2) = bool_hidden
            Else
                If (node_type = "pw-nodes") Then
                    If (output.non_pw_type = "mtDirect" Or output.non_pw_type = "mtRatings" Or output.non_pw_type = "mtStep" Or output.non_pw_type = "mtRegularUtilityCurve") Then
                        output.multi_collapse_default(0) = bool_hidden
                        output.multi_collapse_default(1) = bool_hidden
                        output.multi_collapse_default(3) = bool_hidden
                    Else
                        output.multi_collapse_default(1) = bool_hidden
                        output.multi_collapse_default(2) = bool_hidden
                    End If
        ...
        ElseIf (node_type = "pw-wrt-nodes") Then
            output.multi_collapse_default(4) = bool_hidden
            output.multi_collapse_default(3) = bool_hidden
        End If


    End Function
    Public Shared Sub setCollapseCookies(ByVal stepType As String, ByVal node_type As String, ByVal status As String)
        Dim context As HttpContext = HttpContext.Current
        Dim App = Anytime.AppSession()
        Dim token As String = $"{App.ActiveProject.ProjectGUID }-{stepType}-{node_type}"
        ...
        If App IsNot Nothing Then
            ...
            If isPM Then
                Dim dictionaryKey As String = $"Key{App.ActiveProject.ProjectGUID }_{stepType}"
    ...
    Public Shared Sub RestoreJudgments(ByVal parentId As Integer)
        Dim context As HttpContext = HttpContext.Current
        Dim app = Anytime.AppSession()
        Dim tNode As clsNode = app.ActiveProject.HierarchyObjectives.GetNodeByID(parentId)
        Dim sSessName As String = String.Format(SessJudgments, parentId)

        If tNode IsNot Nothing AndAlso JudgmentsSaved AndAlso context.Session(sSessName) IsNot Nothing Then
            Dim tUserId As Integer = app.ActiveProject.ProjectManager.UserID
            Dim tJudgments As List(Of clsCustomMeasureData) = CType(context.Session(sSessName), List(Of clsCustomMeasureData))
            Dim dt As DateTime? = Nothing
            If context.Session(SessDatetime) IsNot Nothing Then dt = Convert.ToDateTime(context.Session(SessDatetime))
    ...
    Public Shared Function GetResultsPipeStepData(ByVal data As String, ByVal thisIsBad As Boolean) As Boolean
        Dim context = HttpContext.Current
        Dim app = Anytime.AppSession()
        Dim Model = CType(context.Session(Constants.SessionModel), DataModel)
        Dim retVal As Boolean = False
        Dim lines As String() = data.Split(CType(vbNewLine, Char()))
        Dim PM As clsProjectManager = app.ActiveProject.ProjectManager
        Dim tNode As clsNode = app.ActiveProject.HierarchyObjectives.GetNodeByID(Model.ParentID)
        Dim tUserID As Integer = app.ActiveProject.ProjectManager.UserID

        If Not thisIsBad Then
            tNode.Judgments.DeleteJudgmentsFromUser(tUserID)
        End If
        ...
        If tNode IsNot Nothing AndAlso IsPairwiseMeasureType(tNode.MeasureType()) Then
            ...
            For i As Integer = 0 To Model.ObjectivesData.Count - 1
                ...
                If i < lines.Length Then
                    ...
                    Dim vals As String() = line.Split(CType(vbTab, Char()))

                    For j As Integer = i + 1 To Model.ObjectivesData.Count - 1

                        If j < vals.Length Then
                            Dim v As Double = 0

                            If StringFuncs.String2Double(vals(j), v) AndAlso v <> 0 Then
    ...
    Public Shared Function Getnodes(ByVal [step] As Integer) As List(Of Object)
        Dim context As HttpContext = HttpContext.Current
        Dim app = Anytime.AppSession()
        Dim project = app.ActiveProject
        Dim tAction As clsAction = GetAction([step])
        Dim parentNode = CType(Nothing, clsNode)
        Dim firstNode = CType(Nothing, clsNode)
        Dim secondNode = CType(Nothing, clsNode)
        Dim objHierarchy = CType(project.ProjectManager.Hierarchy(project.ProjectManager.ActiveHierarchy), clsHierarchy)
        Dim altsHierarchy = CType(project.ProjectManager.AltsHierarchy(project.ProjectManager.ActiveAltsHierarchy), clsHierarchy)
        Dim wrtleft = ""
        Dim wrtright = ""
        Dim leftnodeinfo = ""
        Dim rightnodeinfo = ""
        ...
        If tAction IsNot Nothing Then
            ...
            Select Case tAction.ActionType
                Case ActionType.atInformationPage
                    ...
                    list1.Add("")
                Case ActionType.atPairwise
                    Dim actionData = CType(tAction.ActionData, clsPairwiseMeasureData)
                ...
                Case ActionType.atAllPairwise, ActionType.atAllPairwiseOutcomes
                    ...
                    If TypeOf tAction.ActionData Is clsAllPairwiseEvaluationActionData Then
                ...
                Case ActionType.atShowLocalResults
                    Dim localresultsdata = CType(tAction.ActionData, clsShowLocalResultsActionData)
                    list1.Add("")
                    list1.Add("")
                    list1.Add("")
                    list1.Add("")
                ...
                Case ActionType.atShowGlobalResults
                    ...
                    list1.Add("")
                Case ActionType.atNonPWOneAtATime
                    Dim actionData2 = CType(tAction.ActionData, clsOneAtATimeEvaluationActionData)

                    If actionData2.Node IsNot Nothing AndAlso actionData2.Judgment IsNot Nothing Then

                        Select Case (CType(tAction.ActionData, clsNonPairwiseEvaluationActionData)).MeasurementType
                            Case ECMeasureType.mtRatings
                                Dim nonpwdata As clsNonPairwiseMeasureData = CType(actionData2.Judgment, clsNonPairwiseMeasureData)

                                If actionData2.Node.IsAlternative Then
                            ...
                            Case ECMeasureType.mtDirect
                                parentNode = actionData2.Node
                                Dim directData = CType(actionData2.Judgment, clsDirectMeasureData)
                                childNode = If(parentNode.IsTerminalNode, altsHierarchy.GetNodeByID(directData.NodeID), objHierarchy.GetNodeByID(directData.NodeID))
                                list1.Add(childinfo)
                                list1.Add(parentinfo)
                                list1.Add(wrtinfo)
                                list1.Add("")
                            ...
                            Case ECMeasureType.mtStep
                                parentNode = actionData2.Node
                                Dim stepData = CType(actionData2.Judgment, clsStepMeasureData)
                                childNode = If(parentNode.IsTerminalNode, altsHierarchy.GetNodeByID(stepData.NodeID), objHierarchy.GetNodeByID(stepData.NodeID))
                                childinfo = InfodocService.Infodoc_Unpack(app.ProjectID, app.ActiveProject.ProjectManager.ActiveHierarchy, If(childNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), childNode.NodeID.ToString(), childNode.InfoDoc, True, False, -1)
                                parentinfo = InfodocService.Infodoc_Unpack(app.ProjectID, app.ActiveProject.ProjectManager.ActiveHierarchy, If(parentNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), parentNode.NodeID.ToString(), parentNode.InfoDoc, True, False, -1)
                            ...
                            Case ECMeasureType.mtRegularUtilityCurve
                                parentNode = actionData2.Node
                                Dim ucData = CType(actionData2.Judgment, clsUtilityCurveMeasureData)
                                childNode = If(parentNode.IsTerminalNode, altsHierarchy.GetNodeByID(ucData.NodeID), objHierarchy.GetNodeByID(ucData.NodeID))
                                childinfo = InfodocService.Infodoc_Unpack(app.ProjectID, app.ActiveProject.ProjectManager.ActiveHierarchy, If(childNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), childNode.NodeID.ToString(), childNode.InfoDoc, True, False, -1)
                                parentinfo = InfodocService.Infodoc_Unpack(app.ProjectID, app.ActiveProject.ProjectManager.ActiveHierarchy, If(parentNode.IsAlternative, Consts.reObjectType.Alternative, Consts.reObjectType.Node), parentNode.NodeID.ToString(), parentNode.InfoDoc, True, False, -1)
                                wrtinfo = InfodocService.Infodoc_Unpack(app.ProjectID, app.ActiveProject.ProjectManager.ActiveHierarchy, Consts.reObjectType.AltWRTNode, childNode.NodeID.ToString(), app.ActiveProject.ProjectManager.InfoDocs.GetNodeWRTInfoDoc(childNode.NodeGuidID, parentNode.NodeGuidID), False, True, parentNode.NodeID)
                                list1.Add(childinfo)
                ...
                Case ActionType.atNonPWAllChildren, ActionType.atNonPWAllCovObjs
                    Dim nowPwAll As clsNonPairwiseEvaluationActionData = CType(tAction.ActionData, clsNonPairwiseEvaluationActionData)

                    Select Case nowPwAll.MeasurementType
                        Case ECMeasureType.mtRatings

                            If TypeOf tAction.ActionData Is clsAllChildrenEvaluationActionData Then
                                Dim data As clsAllChildrenEvaluationActionData = CType(tAction.ActionData, clsAllChildrenEvaluationActionData)
                            ...
                            If TypeOf tAction.ActionData Is clsAllCoveringObjectivesEvaluationActionData Then
                                Dim data As clsAllCoveringObjectivesEvaluationActionData = CType(tAction.ActionData, clsAllCoveringObjectivesEvaluationActionData)
                        ...
                        Case ECMeasureType.mtStep
                            Dim multiNonPwData As clsAllChildrenEvaluationActionData = CType(tAction.ActionData, clsAllChildrenEvaluationActionData)
                            ...
                            list1.Add("")
                        Case ECMeasureType.mtRegularUtilityCurve
                            Dim multiNonPwData1 As clsAllChildrenEvaluationActionData = CType(tAction.ActionData, clsAllChildrenEvaluationActionData)
    ...
    Public Shared Function GetNodeByGuid(ByVal nodeGuid As Guid) As clsNode
        Dim node As clsNode = Nothing

        If Not nodeGuid.Equals(New Guid()) Then
            Dim App = Anytime.AppSession()
            node = App.ActiveProject.HierarchyAlternatives.GetNodeByID(nodeGuid)
            node = If(node, App.ActiveProject.HierarchyObjectives.GetNodeByID(nodeGuid))
        End If

        Return node
    End Function

    Public Shared Function GetNodeTypeAndName(ByVal node As clsNode, ByVal Optional maxLength As Integer = 40) As String
        Dim nodeTypeAndName As String = String.Empty

        If node IsNot Nothing Then
            nodeTypeAndName = String.Format("{0} '{1}'", TeamTimeClass.PrepareTask(If(node.IsAlternative, Consts._TPL_COMPARION_ALTS, Consts._TPL_COMPARION_OBJ)), (If(node.NodeName.Length > maxLength, node.NodeName.Substring(0, maxLength), node.NodeName)))
        End If
