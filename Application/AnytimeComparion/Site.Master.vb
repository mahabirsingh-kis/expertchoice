...
#Region "Page load"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If App.ActiveUser Is Nothing And Not Request.FilePath.Contains("you-are-done.aspx") Then
            Response.Redirect("~/")
        End If

        'If Session("topHeader") IsNot Nothing And CType(Session("topHeader"), Boolean) = True Then
        '    If Session("ViewAsEvaulator") IsNot Nothing And CType(Session("ViewAsEvaulator"), Boolean) = True Then
        '        hdnView.Value = Session("ViewAsEvaulator").ToString()
        '    End If
        'End If
    ...
    Public Shared Function UserWorkSpaceUpdateLastProjectOnly() As Boolean
        ...
        Try
            App.Database.ExecuteSQL("UPDATE userworkgroups SET LastProjectID=-1 WHERE UserID='" & App.ActiveUser.UserID.ToString() & "' and WorkGroupID='" + App.ActiveUserWorkgroup.WorkgroupID.ToString() & "'")
            Return True
        Catch
            Return False
        End Try
