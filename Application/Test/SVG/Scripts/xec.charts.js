...
(function ($) {
    $.widget("expertchoice.ecChart", {
        ...
        // return list of changed options, use it to save state of widget
        ...
        _getChartAreaSize: function () {
            var opt = this.options;
            return {
                width: Math.floor(opt.showLegend && (opt.legendPosition == "right" || (opt.legendPosition == "auto" && opt.width > opt.height)) ? opt.width - opt.legendWidth : opt.width), 
                height: Math.floor((opt.showLegend && (opt.legendPosition == "bottom" || (opt.legendPosition == "auto" && opt.width <= opt.height)) ? opt.height - opt.legendHeight - this.fontSize(false) : opt.height))
            }
        },
        _resetChartPosition: function (idx, isSquare, chartsCount) {
            ...
            opt.center.x = Math.floor(opt.maxWidth / 2 + widthShift / cols + (opt.maxWidth + widthShift / cols) * (idx % cols));
            opt.center.y = Math.floor(opt.maxHeight / 2 + heightShift / rows + (opt.maxHeight + heightShift / rows) * (Math.floor(idx / cols))); 

            var lr = (opt.hMaxLevel + 1 - opt.selectedNode.level);
            if (lr == 0) lr = 1;
            if (opt.areaMode) {
                opt.levelRadius = Math.sqrt(Math.pow(opt.maxRadius, 2) / lr)
            } else {
                opt.levelRadius = (opt.maxRadius) / lr
            };
        },
        chartView: function () {
            var opt = this.options;
            ...
            if (opt.userIDs.length > 0) {
                if (this._showComponents() && opt.showAlternatives) {
                    for (var i = 0; i < opt.userIDs.length; i++) {
                        ...
                        var uComps = this._getUserPriorities(userID).comps;
        ...
        _drawColumnsChart: function (idx, maxVal) {
            ...
            if (opt.isRotated && opt.showLabels && !isMinLabelFS) { // shift chart for labels
                ...
                if (labelFS < this.minFontSize()) {labelFS = 0};
            ...
            for (var i = 0; i < totalColumns; i++) {
                ...
                if (opt.isRotated) {
                    ...
                    //var uComps = this._getUserPriorities(user.id).comps;
                    var uComps = [];
        ...
        _drawRoot: function (rootNode, userID) {
            ...
            if (rootLevelGroup) {
                ...
                var labeltext = roundedValue(this._getUsersNodePriority(userID, rootNode.id) * 100, opt.decimals) + "% ";
        ...
        _hoverSegment: function (obj, isHover, fromLegend) {
            ...
            for (var i = 0; i < opt.userIDs.length; i++) {
                ...
                if (isHover) {
                    if (!fromLegend) $(opt.divCanvas).prop("title", roundedValue(this._getUsersNodePriority(obj.uid, obj.value.id) * 100, opt.decimals) + "% " + obj.value.name);
        ...
        _drawSegment: function (x, y, startAngle, endAngle, radius1, radius2, node, userID, color) {
            ...
            var M3 = this._getRadPoint(x, y, middleAngle, ((this._getChildren(node).length > 0 ? opt.maxRadius : radius2) + this.minFontSize()));
            ...
            var text = (node.isCategory ? " " : roundedValue(this._getUsersNodePriority(userID, node.id) * 100, opt.decimals) + "% ");
            if (opt.showLabels) {
                text += node.name;
            };
            if (opt.debugMode) {
            ...
            if (((segmentHeight > fs * 2 + fs / 2) && (segmentAngle * r1DegreeLength > fs * 4)) || (!opt.showLabels && (Math.PI * (middleRadius) / 180 * Math.abs(segmentAngle) > fs))) { //
                ...
                var textDirection = (doRotate ?
                    ((middleAngle + opt.initAngle) % 360 > 180) && ((middleAngle + opt.initAngle) % 360 < 360) :
                    ((middleAngle + opt.initAngle) % 360 > 90) && ((middleAngle + opt.initAngle) % 360 < 270)
                    );
                var lines = [];
                if (!opt.showLabels) {
                    lines.push(text);
                    ...
                    if (doRotate) {
                        lines = this.getMultilineRadialText(txt, function (i) { return radius2 - radius1 - fs * 2 }, maxLines)
                    } else {
                        lines = (textDirection ?
                            this.getMultilineRadialText(txt, function (i) { return Math.PI * (radius1 + i * fs) / 180 * Math.abs(segmentAngle - 3) }, maxLines) :
                            this.getMultilineRadialText(txt, function (i) { return Math.PI * (radius2 - i * fs) / 180 * Math.abs(segmentAngle - 3) }, maxLines)
                        );
                    };
                };
                txt.remove();
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (opt.showLabels) {
                        ...
                        txt.center(M.x, M.y).rotate((middleAngle + opt.initAngle) % 360 < 180 ? middleAngle + opt.initAngle - 90 : middleAngle + opt.initAngle + 90);
                    };
                };
                ...
                if (opt.chartsPerPage == 1) {
                    var labelLine = (M.x < opt.center.x ? M3.x - this.minFontSize() : M3.x + this.minFontSize());
                    segmentGroup.path(["M", M.x, M.y, "L", M3.x, M3.y, "H", labelLine].join(" ")).stroke({ color: "#555555", width: 0.5 }).fill("transparent");
                    labelLine = (M.x < opt.center.x ? M3.x - this.minFontSize() * 1.5 : M3.x + this.minFontSize() * 1.5);
                    segmentGroup.text(text).font({ size: this.minFontSize(), anchor: (M.x < opt.center.x ? "end" : "start") }).fill("#000000").move(labelLine, M3.y - this.minFontSize() / 1.5);
                }
            };
        ...
        _initHierarchy: function (node, level, userID) {
            ...
            if ((children) && (children.length > 0)) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    sumPrty += this._getUsersNodePriority(userID, child.id);
                ...
                if (sumPrty > 0) {
                    for (var i = 0; i < children.length; i++) {
                        var child = children[i];
                        child.percentage = this._getUsersNodePriority(userID, child.id) / sumPrty * 100;
                    }
                }
            }
        },
        _initVisibleNodes: function (node) {
            ...
            if ((children) && (children.length > 0)) {
                for (var i = 0; i < children.length; i++) {
                    var child = children[i];
                    child.hColor = (child.level <= 1 ? child.color : ColorBrightness(node.hColor, 0.3));
        ...
        _drawLegend: function () {
            ...
            if (opt.legendPosition == "right" || (opt.legendPosition == "auto" && w > h)) {    
        ...
        _initShowAlternatives: function () {
            ...
            opt.selectedPage = 0;
            var rootNode = this._resetRootNode();
            opt.selectedNodeID = rootNode.id;
            this._initHierarchy(rootNode, 1, opt.userIDs[0]);
            var nodes = (opt.showAlternatives ? opt.dataSource.alts : opt.dataSource.objs);
            this._sortNodes(opt.sortBy, nodes);
            this._setSelectedNodeByID(opt.selectedNodeID);
        ...
        _getRadAngle: function (angle) {
            var opt = this.options;
            return (Math.PI / 180.0) * (angle + opt.initAngle - 90);
        },
    });
})(jQuery);
