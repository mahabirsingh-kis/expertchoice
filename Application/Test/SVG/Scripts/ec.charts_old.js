    ...
    $.widget("expertchoice.ecChart", {
        ...
        _drawSegment: function (x, y, startAngle, endAngle, radius1, radius2, node, userID, color) {
            ...
            if (((segmentHeight > fs * 4 + fs / 2) && (segmentAngle * r1DegreeLength > fs * 4)) || (!opt.showLabels && (Math.PI * (middleRadius) / 180 * Math.abs(segmentAngle) > fs))) { //
                ...
                for (var i = 0; i < lines.length; i++) {
                    var txt = segmentGroup.text(lines[i]).font({ size: fs }).fill(getContrastColor(color)); //.move(0, -fs * 2) , anchor: (opt.showLabels ? "middle" : "start")
                    if (opt.showLabels) {
                        var radius3 = radius2 - 4 - i * fs; // radius2 - (i + (maxLines - lines.length) / 2) * fs - exportShift;
                        var radius4 = radius1 + 4 + i * fs; // radius1 + (i + (maxLines - lines.length) / 2) * fs + exportShift;
