...
Remove-Module ComparionMod -ErrorAction Ignore -WarningAction SilentlyContinue
Import-Module .\ComparionMod.psm1 -Force -WarningAction SilentlyContinue

$INI = Get-IniContent .\Setup.ini
$startTime = Get-Date

if ($ini.setup.WCERT_encodedvalue -eq ""){
    Write-Host 'Missing Machine Certificate encoded value.  Run Certs\MakeCerts.bat and take result (which will appear in notepad)'
    Write-Host 'and add to setup.ini file, in SECTION "setup", VALUE "WCERT_encodedvalue" then run this script again.'
    Exit 1
}
...
$sections="setup"
Set-Config("build")
if ([System.Convert]::ToBoolean($ini.setup.SqlServerTrusted) -eq 'True'){
    Set-Config("trusted")
}
if ($ini.azure.SubscriptionID -eq ''){
    Set-Config("selfhost")
}

$websites=".\Application", ".\HTML5Application"

foreach($ws in $websites) {
    #REPLACE VARIABLES
    foreach($sec in $sections) {
        foreach($c in Get-ChildItem -Path $ws  -Recurse -Filter *.config) {
...
if ($ini.azure.ServerIP -ne ''){
    Clear-DnsClientCache 
    $dnsresponse = Set-DNSSimpleARecord -subdomain $WebSiteSubDomain -domain $WebSiteDomain -ip $ini.azure.ServerIP -account $ini.dnsimple.AccountId -token $ini.dnsimple.Token
    $dnsresponse = Set-DNSSimpleARecord -subdomain $WebSiteResponsiveSubDomain -domain $WebSiteDomain -ip $ini.azure.ServerIP -account $ini.dnsimple.AccountId -token $ini.dnsimple.Token
    Clear-DnsClientCache 
}
...
{
    .\Sign.ps1
    Copy-Item .\TeamTimeClient\Release\publish\*.* .\Application\TTA -Force
}
Catch
{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU3NWMDfme4kXoamXpdTb7Iflg
# /VWgghQlMIIEfTCCA2WgAwIBAgIDG+cVMA0GCSqGSIb3DQEBCwUAMGMxCzAJBgNV
...
# hkiG9w0BCQQxFgQU+o/xO4he1z0OBvn20hS/l11A0PMwDQYJKoZIhvcNAQEBBQAE
# ggIAciH5O45z32lCJcJvV7iSw8JouZiw9vK6We+3QhaT6tjwrOg+SNAG7/OXiLAz
# 0aKbNfq1Bajz1LSi1eKbR/g260sxK1xbyB3gSlaJ+Y++zpUWyXEc8d2l50ZfVGyT
# sXcytU3ZVrYlZOlkIrIIcB0OJaY64DRV1xI/QKhCpblzapTKWbMArd5Y0kdDs8vY
# rb5+/fADddqs053H1TUQjImDYWMEkZZtDkcHVBykjh/4ONWdxa6qbbjVT4Eyip2t
# nrgaWoxZWFXJ2+uBMavjpQqJsLQY3YO0g7y+bwUlg+Q0YKDz4y3ZxuWuyiDODzdC
# bvs0UxngqAX0moqoTjhhnsrILBVBIgBcHrG816H3ds0gjh3+6lKro7bhwUmyB3mg
# X9aoAnIEzRdG8PCrovu48Suw3YyPCa38sqoLm4IK8ZvjNVqWE48TM3Oei9gqw9Z5
# Ll+aJSo30H8HY7+twavkMb/cIvKJ47ii99SFUW4Bz7ACxIrGE+z4e7nyEnkSUIF2
# AaxSofJxT75FmwbR+Q4h2wDtGtYWx7HOd0pREb3ry7Z94qcf9eP4EcAcEXaMv6S7
# RqhMnkas2HToLHAEI3usVt5VLIklDHYbMoygBbUea0J15J5xlY1ZE5nCJ6sd2DME
# MMZR3wMS3JeHm3TnsI1cKeqHR9zesNwP1QDj4JQ5rP1rd/2hggJDMIICPwYJKoZI
...
# BTEPFw0xODEyMjIxODA2NThaMCMGCSqGSIb3DQEJBDEWBBT/e3SSCYV7aBWKmIk5
# 1l7VvH3gLDANBgkqhkiG9w0BAQEFAASCAQDaJ0exEBgIkoJEZ6+UElTqCP5fKdjZ
# vyC1oo4Fp3kYyPO76elQhdmuqY+HctThs6ug/q3cgnRBXe8otOviAlQu7IXwih9h
# B8BPuiRSUbA0I7UpW/PmXbsdN+kVwCnPZ+VNJpUTrCvFgdwfqxvk+AjE8DxGaQWX
# rhrSk0vwMEhW6NEEMIVu9tk2J21EU5ttsa81KjfmBSl9mZd9X6CArzD05ZGE1zn7
# fZl8P13wGe0u2o37znMwI6hPa3ux5Eyv/uoTFGv0xmKmuQO7as6IX3lbQyn0Wcjb
# Iq2p6+ZD0KG6+z1xhMen2ZFoDMy6NU15gwjUjW8RJF74pQoN2WLiHo4y
